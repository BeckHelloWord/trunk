...
define(function (require,exports,module) {
    ...
    router.map({
        ...
        '/vip': {
            ...
            subRoutes: {
				...
				'/expect': {
					component: function(resolve) {
						require.async(['views/vip/expect.js'],resolve);
					}
				}
            }
        },
		...
		'/center/youpin': {
			component: function(resolve) {
				require.async(['views/center/youpin/title.js'],resolve);
			},
			subRoutes: {
				'/index/:id': {
					component: function (resolve) {
						require.async(['views/center/youpin/index.js'], resolve);
					}
				},
				'/service': {
					component: function (resolve) {
						require.async(['views/center/youpin/service.js'], resolve);
					}
				},
				'/safety': {
					component: function (resolve) {
						require.async(['views/center/youpin/safety.js'], resolve);
					}
				}
			}
		},
		'/brisk': {
			component: function(resolve) {
				require.async(['views/center/brisk_mobile/brisk.js'],resolve);
			},
			subRoutes: {
				'/index': {
					component: function(resolve) {
						require.async(['views/center/brisk_mobile/index.js'],resolve);
					}
				}
			}
		},
	...
	router.redirect({
		...
		//'/vip': '/vip/expect',	//敬请期待
		'/help':'/help/index',
		'/youpin/:id':'/center/youpin/index/:id'
...
//})
	...
	// 注册过滤函数，客户详情页保护敏感信息
	Vue.filter('strTranslated', function (value) {
		if(value.length == 11){
			return value.substr(0,3)+" **** "+value.substr(7,11);
		}else if(value.length < 4&& value.length>0){
			return value.replace(value.slice(0, 1), "* ");
		}else if(value.length>=4){
			return value.replace(value.slice(0, 2), "* ");
		}else{
			return '&nbsp;'
		}
	})
