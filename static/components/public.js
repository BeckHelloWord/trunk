...
define(function (require,exports,module) {
	...
	*/
	var API_GET = function(config){
		//增加客户端调用过settoken的标示 当设置完成后 API_GET才继续进行向服务器请求数据 Auth.get()==0 避免设置storage时延迟造成的提前发送请求
		if((!window.setTokenFlag)){
			//月月丰详情页面客户端 settoken方法遗漏 特殊处理
			if(location.hash.indexOf("center/youpin/index")>-1||location.hash.indexOf("center/youpin/service")>-1){
	...
	var getAppFunc=function(funcName){
		var result;
		try{
			result = JSON.parse(localStorage['appFunc']);
		}catch(r){
			result = sessionStorage['appFunc'] && JSON.parse(sessionStorage['appFunc']);
		}
		if(result&&result.indexOf(funcName)>-1){
			return true
		}else{
			return false
		}
	};
	exports.getAppFunc = getAppFunc;
	
	var getlist = function (arr) {
			var VueExports = arr || publicArr.me;
			var postData = pagedata[publicArr.path];
			VueExports.nowLoading=true
			API_GET({
				url : postData.url,
	...
	$(window).on('scroll',function() {
		...
		if(raw.length == 1){ //加载按钮有且只有一个时执行
			raw.each(function(){
				...
				if ((scrollTop + 120 + windowHeight * 1.8 >= meTop)) {
					pagedata[publicArr.path]['stillMore'] = false;
